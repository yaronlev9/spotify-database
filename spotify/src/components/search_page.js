...
export default function SearchPage(props) {
  ...
  return (
    ...
    {songs.map((item,i)=> 
      <div className="record" key={i}>
          <Link style={{color:'white'}} to={`/song/${item._source.SongID}`}>
              <img alt={item._source.Title} src={item._source.Cover_image} 
              width="20" height="20"></img>
              <span className="title">{i + 1}. {item._source.Title}</span>
              <span className="name">{item._source.Artist_name}</span>
              <span className="length">{str_pad_left(Math.floor(item._source.Length/60),'0',2)+':'+str_pad_left(item._source.Length - (Math.floor(item._source.Length/60)) * 60,'0',2)}</span>
